...
export const Header: React.FC = () => {
  ...
  return (
    <>
      ...
      <div className="fixed top-0 left-0 w-full h-8 bg-[#ff2a14] border-b-2 border-zinc-950 text-white flex items-center overflow-hidden z-50 select-none text-[10px] sm:text-xs font-black uppercase tracking-wider">
        <div className="animate-marquee-custom whitespace-nowrap flex items-center space-x-12 pr-12">
          <span>Khuda lagse? Order Chillox now!</span>
          <span>Hot and Crispy Naga Drums now available!</span>
          <span>Extra cheese block? Boss, add customized choice inside menu!</span>
          <span>Free delivery on orders above 1000 BDT!</span>
          <span>Pankha wings er box, double coke, r bondhuder adda!</span>
          <span>Khuda lagse? Order Chillox now!</span>
          <span>Hot and Crispy Naga Drums now available!</span>
          <span>Extra cheese block? Boss, add customized choice inside menu!</span>
          <span>Free delivery on orders above 1000 BDT!</span>
      ...
      <header
        className={`fixed transition-all duration-300 z-40 left-1/2 -translate-x-1/2 w-[92%] max-w-5xl rounded-[2rem] bg-white border-2 border-zinc-950 shadow-[4px_4px_0px_#000] py-3 px-4 md:px-6 ${
          isVisible ? 'top-12 opacity-100' : '-top-24 opacity-0 pointer-events-none'
        }`}
      >
